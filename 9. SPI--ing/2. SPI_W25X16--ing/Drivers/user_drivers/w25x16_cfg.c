...
#define DEVICE_ID                           0x14        /**< W25X16_DEVICE_ID */
#define MANUFACTURE_ID                      0xEF14      /**< W25X16_MANUFACTURE_DEVICE_ID */
#define JEDEC_ID                            0xEF3015    /**< W25X16_JEDEC_DEVICE_ID */
...
void w25x16_test(void)
{
    ...
    HAL_Delay(1);
    GUI_DispHexAt(Id.memory, 80, 50, 2);
    GUI_DispHexAt(Id.compatibility, 80, 80, 2);
    GUI_DispHexAt(Id.maunfacture, 80, 110, 2);
    GUI_DispHexAt(Id.device, 80, 140, 2);
...
/**
  ...
  */
struct W25X16_ID w25x16_read_id(uint8_t id_select)
{
    ...
    {
        spi_send_onebyte(W25X16_JEDEC_DEVICE_ID);
        Id.maunfacture = spi_send_onebyte(DUMMY_BYTE);
        Id.memory = spi_send_onebyte(DUMMY_BYTE);
        Id.compatibility = spi_send_onebyte(DUMMY_BYTE);
    }
    else if (id_select == 1)    /**< 读取器件ID */
    {
        spi_send_onebyte(W25X16_DEVICE_ID);
        spi_send_onebyte(DUMMY_BYTE);
        spi_send_onebyte(DUMMY_BYTE);
        spi_send_onebyte(DUMMY_BYTE);
        
        Id.device = spi_send_onebyte(DUMMY_BYTE);
    }
    else if (id_select == 2)    /**< 读取制造 & 器件 ID */
    {
        spi_send_onebyte(W25X16_MANUFACTURE_DEVICE_ID);
        /** 发送24位的地址 以8位为单位分三次依次发送（从高字节开始发送） */
        spi_send_onebyte((addr & 0xff0000) >> 16);
        spi_send_onebyte((addr & 0x00ff00) >> 8);
        spi_send_onebyte((addr & 0x0000ff) >> 0);
...
void w25x16_write_status_register(uint8_t written_data)
{
    ...
    spi_send_onebyte(W25X16_WRITE_STATUS_REG);
    
    spi_send_onebyte(written_data);
...
void w25x16_read_data(uint32_t addr)
{
    ...
    spi_send_onebyte(W25X16_READ_DATA);
...
void w25x16_erase(enum W25X16_ERASE EraseArea, uint32_t addr)
{
    ...
    {
        spi_send_onebyte(W25X16_BLOCK_ERASE);
        /** 发送24位的地址 以8位为单位分三次依次发送（从高字节开始发送） */
        spi_send_onebyte((addr & 0xff0000) >> 16);
        spi_send_onebyte((addr & 0x00ff00) >> 8);
        spi_send_onebyte((addr & 0x0000ff) >> 0);
    ...
    {
        spi_send_onebyte(W25X16_SECTOR_ERASE);
         /** 发送24位的地址 以8位为单位分三次依次发送（从高字节开始发送） */
        spi_send_onebyte((addr & 0xff0000) >> 16);
        spi_send_onebyte((addr & 0x00ff00) >> 8);
        spi_send_onebyte((addr & 0x0000ff) >> 0);
    ...
    {
        spi_send_onebyte(W25X16_CHIP_ERASE);
    }
    else
    {
        ;
...
void w25x16_page_program(uint8_t *buffer, uint32_t addr, uint16_t byte_num)
{
    ...
    spi_send_onebyte(W25X16_PAGE_PROGRAM);
    /** 发送24位的地址 以8位为单位分三次依次发送（从高字节开始发送） */
    spi_send_onebyte((addr & 0xff0000) >> 16);
    spi_send_onebyte((addr & 0x00ff00) >> 8);
    spi_send_onebyte((addr & 0x0000ff) >> 0);
...
void w25x16_set_power(FunctionalState state)
{
    ...
    if (state == DISABLE)
    {
        spi_send_onebyte(W25X16_POWER_DOWN);
    }
    else 
    {
        spi_send_onebyte(W25X16_RELEASE_POWER_DOWN);
    }
...
static void w25x16_set_write_enable(FunctionalState state)
{
    ...
    if (state == DISABLE)
    {
        spi_send_onebyte(W25X16_WRITE_DISABLE);
    }
    else
    {
        spi_send_onebyte(W25X16_WRITE_ENABLE);
    }
...
static void w25x16_read_status_register(void)
{
    ...
    spi_send_onebyte(W25X16_READ_STATUS_REG);
    
    do
    {
        /** 发送伪字节以得到BUSY的值 */
        status = spi_send_onebyte(DUMMY_BYTE);
    }
