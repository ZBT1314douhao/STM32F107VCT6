...
static uint16_t GPIO_read_port(GPIO_TypeDef* GPIOx);
static void GPIO_write_port(GPIO_TypeDef* GPIOx, uint16_t PortVal);
...
void lcd_clr(uint16_t color)
{
	...
	{
		lcd_clr_wr();
		GPIO_write_port(LCD_DATA_PORT, color);
		lcd_set_wr();		
	}
	lcd_set_cs();   
}	
...
static void lcd_write_reg(uint16_t reg,uint16_t value)
{
	...
	lcd_clr_rs();
	lcd_clr_wr();
	GPIO_write_port(LCD_DATA_PORT, reg);
	lcd_set_wr();

	lcd_set_rs();
	lcd_clr_wr(); 
	GPIO_write_port(LCD_DATA_PORT, value);
	lcd_set_wr();
	lcd_set_cs();
}
...
static void lcd_ram_prepare(void)
{
	...
	lcd_clr_rs();
	lcd_clr_wr();	 
	GPIO_write_port(LCD_DATA_PORT, 0x22);
	lcd_set_wr();
	lcd_set_cs();
}
...
static uint16_t lcd_read_sta(void)
{
	...
	lcd_clr_rd(); 
	lcd_set_rd(); 	
    value = GPIO_read_port(LCD_DATA_PORT); 
	lcd_set_cs();
 ...
 *  @return None
 */
void lcd_write_addr(uint16_t addr)
{
	lcd_clr_cs();
	lcd_clr_rs();
	lcd_clr_wr();
	GPIO_write_port(LCD_DATA_PORT, addr);
	lcd_set_wr();
    lcd_set_cs();
}
...
void lcd_write_data(uint16_t dat)
{
	...
	lcd_set_rs();
	lcd_clr_wr(); 
	GPIO_write_port(LCD_DATA_PORT, dat);
	lcd_set_wr();
	lcd_set_cs();
}
...
void lcd_read_data_multiple(uint16_t *p_dat, uint16_t num_items)
{
    ...
    while(num_items--)
    {
        *p_dat++ = GPIO_read_port(LCD_DATA_PORT); 
    }
	lcd_set_cs();
	lcd_data_as_output();
}
...
uint16_t lcd_read_data(void)
{
	...
	lcd_set_rd();
    lcd_data_as_input();
    read_data = GPIO_read_port(LCD_DATA_PORT);
    lcd_set_cs();
	lcd_data_as_output();
