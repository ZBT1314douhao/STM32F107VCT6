...
/** 控制引脚设置 */
//#define lcd_set_cs()	HAL_GPIO_WritePin(LCD_CS_PORT, LCD_CS_PIN, SET)
//#define lcd_set_wr()	HAL_GPIO_WritePin(LCD_WR_PORT, LCD_WR_PIN, SET)
//#define lcd_set_rs()	HAL_GPIO_WritePin(LCD_RS_PORT, LCD_RS_PIN, SET)
//#define lcd_set_rd()	HAL_GPIO_WritePin(LCD_RD_PORT, LCD_RD_PIN, SET)
//#define lcd_reset_cs()	HAL_GPIO_WritePin(LCD_CS_PORT, LCD_CS_PIN, RESET)	
//#define lcd_reset_wr()	HAL_GPIO_WritePin(LCD_WR_PORT, LCD_WR_PIN, RESET)
//#define lcd_reset_rs()	HAL_GPIO_WritePin(LCD_RS_PORT, LCD_RS_PIN, RESET)
//#define lcd_reset_rd()	HAL_GPIO_WritePin(LCD_RD_PORT, LCD_RD_PIN, RESET)	

static void lcd_set_cs(void);	
static void lcd_set_wr(void);
static void lcd_set_rd(void);
static void lcd_set_rs(void);
static void lcd_reset_rd(void);
static void lcd_reset_rs(void);
static void lcd_reset_wr(void);
static void lcd_reset_cs(void);
...
static void lcd_pin_config(void)
{
	...
	GPIO_InitStructure.Pin = GPIO_SPEED_LOW;
	GPIO_InitStructure.Pin  = GPIO_MODE_OUTPUT_PP;
...
static uint16_t lcd_read_sta(void)
{
    ...
    for (i = 0; i < 15; i++)
    {
        value |= HAL_GPIO_ReadPin(LCD_DATA_PORT, GPIO_PIN_0);
    }
	lcd_set_cs();
...
static void lcd_set_cs()
{
    HAL_GPIO_WritePin(LCD_CS_PORT, LCD_CS_PIN, GPIO_PIN_SET);
}        
static void lcd_set_wr()
{
    HAL_GPIO_WritePin(LCD_WR_PORT, LCD_CS_PIN, GPIO_PIN_SET);
}
static void lcd_set_rd()
{
    HAL_GPIO_WritePin(LCD_RD_PORT, LCD_CS_PIN, GPIO_PIN_SET);
}
static void lcd_set_rs()
{
    HAL_GPIO_WritePin(LCD_RS_PORT, LCD_CS_PIN, GPIO_PIN_SET);
}

static void lcd_reset_rd()
{
    HAL_GPIO_WritePin(LCD_RD_PORT, LCD_RD_PIN, GPIO_PIN_RESET);
}
static void lcd_reset_rs()
{
    HAL_GPIO_WritePin(LCD_RD_PORT, LCD_RD_PIN, GPIO_PIN_RESET);
}
static void lcd_reset_wr()
{
    HAL_GPIO_WritePin(LCD_RD_PORT, LCD_RD_PIN, GPIO_PIN_RESET);
}
static void lcd_reset_cs()
{
    HAL_GPIO_WritePin(LCD_RD_PORT, LCD_RD_PIN, GPIO_PIN_RESET);
}
