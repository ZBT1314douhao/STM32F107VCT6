        ...
        case WM_NOTIFICATION_RELEASED:      // React only if released
          ...
          {       // Toggle color
            if (_Color) 
            {
              ...
              BUTTON_SetBkColor(hButton, 1, GUI_WHITE);     /**< 按钮被按下时按钮的背景色 */
              BUTTON_SetTextColor(hButton, 0, GUI_BLACK);
              BUTTON_SetTextColor(hButton, 1, GUI_BLACK);   /**< 按钮被按下时文本的背景色 */
            } 
            else 
            {
...
void button_round_test(void) 
{
  ...
  {
    _Font = _Color = 0;
      /** (对话框所包含小工具资源表的指针， 对话框中小工具的个数， 对话框过程函数指针，
            父窗口的句柄(0表示无父窗口), 对话框相对于父窗口的x，y轴位置); 
      */
    GUI_ExecDialogBox(_aDialogCreate, GUI_COUNTOF(_aDialogCreate), &_cbDialog, 0, 0, 0);          /**< 创建并执行对话框 */
    GUI_Delay(1000);
  }
}
